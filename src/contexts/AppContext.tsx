...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const loadProfileData = (profile: UserProfile) => {
      ...
      if (profile === "Lucas") {
        ...
        if (!localStorage.getItem(`pixel-tracker-userstats-${profile}`) && localStorage.getItem("pixel-tracker-userstats")) {
          localStorage.setItem(`pixel-tracker-userstats-${profile}`, localStorage.getItem("pixel-tracker-userstats")!);
        }
      }
      ...
      // Load UserStats
      const storedUserStats = localStorage.getItem(`pixel-tracker-userstats-${profile}`);
      if (storedUserStats) {
        try {
          setUserStatsState(JSON.parse(storedUserStats));
        } catch (e) {
  ...
  const setUserStats = (stats: UserStats) => {
    ...
    localStorage.setItem(`pixel-tracker-userstats-${activeProfile}`, JSON.stringify(stats));
  };
